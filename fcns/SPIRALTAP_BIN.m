...
function [x, varargout] = SPIRALTAP_BIN(y, A, tau, pulse_num, varargin)
...
if sum( strcmpi(noisetype,{'poisson','gaussian','binomial'})) == 0
...
Axprevious = Ax;
xprevious = x;
grad = computegrad(y,Ax,AT,pulse_num,noisetype,logepsilon);
...
if saveobjective
    ...
    objective(iter) = computeobjective(x,y,Ax,tau,pulse_num,noisetype,logepsilon,penalty,WT);
end
if savereconerror
        ...
        case 1 % Barzilai-Borwein choice of alpha
            if monotone 
                ...
                while (accept == 0)
                    ...
                    objective(iter + 1) = computeobjective(x,y,Ax,tau,pulse_num,...
...
% =========================
function grad = computegrad(y,Ax,AT,pulse_num,noisetype,logepsilon)
    ...
    switch lower(noisetype)
        ...
        case 'gaussian'
            ...
            grad(mask) = 0;
        case 'binomial'
            grad = AT((pulse_num-y)./pulse_num - (y./(Ax + logepsilon)));
    end
end
...
% ==========================
function objective = computeobjective(x,y,Ax,tau,pulse_num,noisetype,logepsilon,...
    penalty,varargin)
% Perhaps change to varargin 
...
switch lower(noisetype)
    ...
    case 'gaussian'
        mask = (y ~= 0); % trusted data
        y = y(mask);
        Ax = Ax(mask);
        objective = sum( (y(:) - Ax(:)).^2)./2;
    case 'binomial'
        precompute = y.*log(Ax + logepsilon);
        scales = (pulse_num-y)./pulse_num;
        objective = sum(scales(:).*Ax(:)) - sum(precompute(:));
end
% 2) Compute Penalty:
switch lower(penalty)
    case 'canonical'
